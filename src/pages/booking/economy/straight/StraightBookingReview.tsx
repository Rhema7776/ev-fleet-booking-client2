...
const StraightBookingReview = () => {
  ...
  const handleSubmit = async () => {
    console.log("booking state at submit:", booking);
    ...
    try {
      ...
      const created = await createBooking({
        bookingMode: booking.bookingMode ?? "STRAIGHT",
      ...
      navigate("/booking/economy/straight/confirmed", { state: { booking: created } });
    } catch (err) {
      const axiosError = err as AxiosError<{ message?: string }>;
      setError(axiosError.response?.data?.message || "Unable to submit booking request.");
    } finally {
      setSubmitting(false);
    }
  };
...
export default StraightBookingReview;
